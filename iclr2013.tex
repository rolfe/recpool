...
% MAKE CLEAR THAT E AND D ARE NOT TIED
% EMPHASIZE THAT PART/TEMPLATE DECOMPOSITION IS DATA-DRIVEN
...
%We unroll an implicit sparse coder into an explicit deep network, and fine-tune using a discriminative classification loss in addition to a sparse autoencoder loss.  

\begin{abstract}
%Many natural datasets consist of a low-dimensional manifold within a high-dimensional ambient space, with class identity stable along the data manifold but more rapidly varying in the ambient space.
%This structure can exploited using an input decomposition into components within and orthogonal to the data manifold, but existing approaches to produce such decompositions are inefficient. % and biologically implausible.  
%Deep networks have proven to be powerful, but they fail to efficiently exploit the way in which the local structure present in sensory stimuli can be used to resolve ambiguities.
\begin{comment}
...
From an initially unstructured recurrent network, the hidden units of discriminative recurrent sparse auto-encoders naturally organize into a hierarchy of features. The systems spontaneously learns categorical-units, whose activity build up over time through interactions with part-units, which represent deformations of templates.  
% The most abstract units use a sophisticated form of pooling to gradually integrate 
% over the lower-level units, and implement templates for each supervised class.  The lower-level units directly construct a sparse, part-based decomposition of the residual input after the template is subtracted, distinguishing between directions parallel and orthogonal to the data manifold.  %and thus measure a low-dimensional tangent distance around the template.  
Even using a small number of hidden units per layer, discriminative recurrent sparse auto-encoders that are pixel-permutation agnostic achieve excellent performance on MNIST.
...
2. This is because three things: 
    the complex interaction between multiple alternative interpretations
    the highly non-linear nature of the decision surfaces
    world has hierarchical, part-based structure)

Deep architectures complement this structure
    enable complex decisions require several steps to compute a sensible answer. 
\end{comment}


\begin{comment}
%Many sensory problems of interest are locally ambiguous, but globally clear-cut.   After zooming out, all observers will consistently discriminate between a cat and a dog.  Nevertheless, the global information cannot be efficiently tackled in a monolithic manner.  

%The data manifold of sensory problems convoluted, and cannot be broken into semantically meaningful regions by simple decision surfaces like planes.  It is impractical, both in terms of computation and data, to attempt to build models that can capture all highly convoluted manifolds; they have too many parameters to be trained.  It is thus necessary to build a priori knowledge about the structure of the sensory world into the model.  


%Many natural stimuli of practical interest have a hierarchical structure.  For instance, human beings are composed of a head, a torso, two arms, and two legs; each arm comprises an upper arm, a forearm, and a hand; etc.  
...
%Traditionally, deep networks have used independent parameters for each layer~\citep{hinton2006}.  While such an unconstrained parameterization is certainly sufficient, it fails to capture much of the structure of the natural world.  %is likely to be unnecessarily unstructured.  
%In particular, it precludes the sharing of information throughout the hierarchy.  %When trained in a purely discriminative manner, deep neural networks are subject to the vanishing gradients problem (cite Bengio).  We need to identify another piece of the structure of the sensory world.

However, at any given level of a hierarchy, the features are often ambiguous when evaluated individually. Even tasks as basic as object segmentation and phoneme recognition are not well-posed when considered at the local level.  
%The information from the individual parts must be integrated together to achieve a reliable assessment of the whole.  
%
%In contrast, global sensory problems are often clear-cut; human observers can reliably segment and identify visual objects, and recognize speech. % , despite the impossibility of distinguishing any small region of the animal from a similarly-colored background.  
%If the parts were conditionally independent given the whole, this could be done in a simple product of conditional probabilities.  In practice, details of the representation must be discarded at each layer of the hierarchy, lest the representation explode exponentially.  
%As a result, the parts are strongly coupled, and in a manner that depends on more than just the higher-level classification.  
%When the representation of two edges is composed into that of a line segment, the exact position and orientation of each component edge is not retained.  The segments that make up an edge are adjacent and aligned.  The expression around the eyes is correlated with the expression around the mouth.  Each phoneme is modified by those surrounding it to give rise to a panoply of phones.  
%To disambiguate each part, it is thus necessary to integrate information from the surrounding parts. %that is naturally discarded further up the hierarchy due to representational constraints.  
%If a part is to be disambiguated based upon its neighbors n out, this requires a large receptive field and complicated decision surface.  The problem is that the neighboring parts upon which the disambiguation is to be performed are themselves ambiguous.  
%To take advantage of the local correlations between parts, it is natural to build networks that gradually resolve ambiguity and refine answers within each level of the hierarchy, before the necessary detail is discarded.  
%Each successive computation in such an ambiguity-resolution procedure will be similar to the others; the natural representation doesn't change, but the network improves the accuracy of each component as information propagates throughout the representation.  
While this ambiguity could be resolved directly via reference to the global input, the task can be simplified by taking advantage of the hierarchical structure of natural stimuli. % once again.  
%Most combinations of part (identities) are not valid, and so 
The identity of each feature is strongly correlated with the identity of complementary features at the same level of the hierarchy. %, rather than the full details of the input.
%local structure in space and time of natural stimuli.  The identity of each part is primarily influenced by the identities of the nearby parts.
%The identity of each component depends upon those of its neighbors, which in turn depend upon the identity of their neighbors.  
 Ambiguity can thus be resolved efficiently via a recursive procedure, in which the current estimate of each feature's identity is based upon the previous estimates of the correlated features, and information is allowed to gradually propagate laterally throughout the entire representation.
...
%Recurrent computation underlying sparse coding resolves the ambiguity regarding which of a set of similar, mutually exclusive parts best explains the input.  

It is not necessary to choose between hierarchical and recurrent network organization: a hierarchical computational structure can exist within a recurrent network.   %If only a subset of hidden units is active at any one time, 
Specifically, a recurrent network can dynamically select a small subset of active units at each time step, and thereby organize itself into virtual ``layers'' of coactive units in a data-dependent manner.  
%Each such virtual layer can potentially perform a different operation, as in the original hierarchical network.  
...
Representations in which only a fraction of the units are permitted to be simultaneously active are called sparse, and can be computationally powerful~\citep{mairal2012}.  The sparsity constraint can be enforced via biologically plausible mechanisms \citep{rozell2008}, and is consistent with the neural activity observed in the cortex \citep{vinje2000, olshausen2004, lewicki2002, smith2006, saxe2011}.  %Traditional sparse coding is powerful (mairal), but computationally inefficient, since it requires many iterations to converge (gregor?).  

In light of these considerations, we study discriminatively trained sparse recurrent networks of rectified linear units~\citep{glorot2011}.  The resulting \emph{discriminative recurrent sparse autoencoder} is similar to the Learned Iterative Shrinkage and Thresholding Algorithm (LISTA)~\citep{gregor2010}, but with a positivity constraint and a different loss function.  Discriminatively trained LISTA autoencoders have been investigated recently, but were trained using a separate encoder for each class, and did not capture the hierarchical structure of the data~\citep{sprechmann2012a, sprechmann2012b}.

%surprise: the representation learned spontaneously by the network consists of a small number of categorical units (whose decoding basis looks like a template for the category and encoding basis looks semi-random) and "modifier" units which reconstruct the input when used to modify the categorical unit.  Proof of principle that we can get hierarchical organization from within a recurrent architecture.  Also yields good performance with few units.  

In this paper, we show that discriminative fine-tuning of a recurrent sparse autoencoder %is sufficient to induce an explicit tangent space decomposition of the input.  Our discriminative fine-tuning procedure 
does not just discover ``parts'' that are more discriminative.  Rather, a hierarchical representation naturally emerges from a homogeneous pool of hidden units, with global template categorical-units slowly integrating %pooling 
...
%One computational advantage of deep architectures over shallow ones is the ability to perform multiple steps of computation to arrive at a decision~\citep{bengio2009}. This is particularly useful when the input is ambiguous, and the decision must result from interactions between alternative interpretations. It is also useful when the input must be transformed into a standardized prototype that can easily be identified. Lastly, a multi-step sequential computation is useful when parts can be combined in a combinatorial way to form a recognizable pattern.
...
%[insert comment and reference to Sprechman, Mairal, Bagnell and Boureau (she had a discriminative sparse coder too)]
...
%Given a characterization of the low-dimensional data manifold and the class of a subset of datapoints, it is natural to categorize a new point based upon the class of its nearest neighbors as measured along the manifold.
%The relevant distance when categorizing new points is distance along the manifold.  
%The directions tangent to the data manifold define the tangent space.
%The linearization of the set of valid deformations at a given datapoint constitutes a basis for the tangent space, which is a local linear approximation of the low-dimensional manifold.  

%Classification using k-nearest-neighbors in the tangent space yields state-of-the-art performance \citep{simard1993, rifai2011b}.  
%Make clear the connection between sparse coding and tangent space representations.
...
However, the standard implementations of these methods implicitly assume a position-independent distance metric, and is thus inefficient.  %uses high-dimensional subspaces surrounding templates, but low-dimensional subspaces are more effective.  
...
\begin{figure}[tb]
  \begin{center}
    \includegraphics[width=5.5in, keepaspectratio=true]{architecture_figure_norm_cropped.pdf}
  \end{center}
...
We consider %deep hierarchical networks 
DrSAEs of rectified linear units with the architecture shown in figure~\ref{architecture_figure}: % in which each hidden layer also receives a projection from the input:
\begin{equation} \label{layer-dynamics}
\x^{t+1} = \max\left(0, \E \cdot \y + \Sm \cdot \x^t - \bv \right)
\end{equation}
for $t = 1, \ldots, T$, where $n$-dimensional vector $\x^t$ is the activity of the hidden units at iteration $t$, %\footnote{The network can also be thought of as flat but recurrent, with $t$ specifying the time step.} $t$,
...
algorithm for minimizing the $\ell_1$-regularized reconstruction loss %at iteration $t=T$
...
Discriminative fine-tuning naturally induces the hidden units to differentiate into a hierarchy-like continuum.  On one extreme are part-units, which perform an ISTA-like sparse coding computation; on the other are categorical-units, which use a sophisticated form of pooling to integrate %pool 
over matching part-units, %are suppressed according to the $\ell_1$ distance orthogonal to the data manifold between their template and the input, 
...
%When trained on MNIST, part-units have localized, pen stroke-like decoders, weak outgoing classification connections, and incoming connections consistent with the ISTA algorithm.  Categorical-units, in contrast, have whole-digit template-like decoders, strong outgoing classification connections, poorly aligned encoders and decoders, and disproportionately strong incoming recurrent.  

% most easily distinguished by the strength of their connections to the classification dictionary, the strength of their recurrent connections, and the alignment of their encoders with their decoders.  
%This can be seen in the plots of figure~\ref{two_classes_figure}, which contain a dense cloud of part-units for which the angle between the encoder row and decoder column is very small, indicating that the two are well-aligned, and the incoming recurrent and outgoing classification connections are weak.  The plots of figure~\ref{two_classes_figure} also contain an extended tail of categorical-units, which have a larger angle between the encoder row and decoder column, and stronger incoming recurrent and outgoing classification connections.  




%categorical-units have strong recurrent connections, encoders that are not aligned with their decoders, and template-like decoders.  Part units perform a sparse decomposition of the residual input with the templates subtracted according to the standard ISTA approach.  As a result, their rows of $\E$ and $\Sm - \I$ proportional to $\D^{\top}$ and $\D^{\top} \cdot \D$, respectively, as consistent with equation~\ref{ISTA-restriction} and as shown in figure~\ref{ista_weight_figure}.

%which have strong recurrent connections, encoders that are not aligned with their decoders, and template-like decoders

%and thus explicitly encode the tangent distance.
...
%This space of perturbations around a template produced by the part-units corresponds to the tangent space at an exemplar.  
...
\begin{figure}[tb] %p
  \begin{center}
    \begin{tabular}{cc}
      \includegraphics[width=2.625in, keepaspectratio=true]{scat_ista_weights_part_to_part.png} &
      \includegraphics[width=2.625in, keepaspectratio=true]{scat_ista_weights_categorical_to_part.png} 
    \end{tabular}
  \end{center}
  \caption{Connection weight versus ista ideal. \label{ista_weight_figure}}
\end{figure}
\end{comment}
...
Given that some units are going to be very large and categorical, they might as well represent the template.  Bounding the $\ell_2$ norm of the weights is similar to standard $\ell_2$ regularization, and is mentioned in Hinton's A Practical Guide to Training Restricted Boltzmann Machines.  
...
\begin{comment}
    ...
    \begin{tabular}{p{0.02in}p{2.4in}p{0.02in}p{2.4in}}
      \parbox[b]{0in}{(a) \vspace{0.7cm}} & \includegraphics[width=2.5in, keepaspectratio=true]{shrink_dictionary_part.png} &
      \parbox[b]{0in}{(b) \vspace{0.7cm}} & \includegraphics[width=2.5in, keepaspectratio=true]{shrink_dictionary_categorical.png} 
    \end{tabular}
\end{comment}
...
Categorical-units implement winner-take-all-like dynamics amongst themselves, as shown in figure~\ref{categorical_connection_figure}(b), with negative connections to most other categorical-units.  Figure~\ref{categorical_connection_figure}(b) plots $\Sm - \I$, since otherwise the strong, positive self-connections of the categorical-units would dwarf all other connections.  These positive self-connections facilitate the integration of input %activity 
over time.
...
As shown in figure~\ref{categorical_connection_statistics_figure}(d), part-units receive most of their input from other part-units; categorical-units receive a larger fraction of their input from other categorical-units.  %Almost every part-unit projects to the most template-like categorical-units.  
...
% Statistics of connections figure
...
\begin{figure}[tb]
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Discriminative recurrent sparse auto-encoders decompose the input into a template and deformations along the data manifold. %tangent space.  
The progressive reconstruction of selected inputs by the hidden representation before (a) or after (b,c) discriminative fine-tuning.  %The top two plots demonstrate the difference between part (a) and template (b) based reconstruction on a variety of different classes.  Plot (c) shows the reconstruction of three instances of the same class from a common template.  
...
then the weighted sum of negative projections from the part units to the selected categorical-unit is the $\ell_1$ tangent distance.  %hidden units that have decoder parts that are orthogonal to the tangent space of a template \emph{is} the tangent
...
A prohibitive amount of data is required to fully characterize the data manifold \citep{narayanan2010}, so it is often approximated by the set of linear submanifolds tangent to the data manifold at the observed datapoints, known as the \emph{tangent spaces} \citep{simard1998, rifai2011b, ekanadham2011}.  %Even using this approximation, evaluating the tangent distance\footnote{The tangent distance between two points is the infimum of the distance between the linear submanifolds tangent to the data manifold at the points.} between two points requires solving a system of $O(n)$ linear equations, where $n$ is the dimensionality of the data manifold.  For each test point, a nearest-neighbors algorithm must compute such a distance for each element of the training set, and is thus computationally inefficient. % and biologically implausible.  
DrSAEs can naturally and efficiently form an explicit tangent space representation, consisting of a template and a perturbation within the data manifold.  %in the associated tangent space.
%While, contractive autoencoders \citep{rifai2011a} and other tangent propagation techniques \citep{simard1993} implicitly learn to only represent the tangent space, they ?!?
...
%discriminative recurrent sparse autoencoders learn to produce a decomposition of the input into a template and a set of perturbations.
...
$\ell_1$ regularization on a hierarchical network of rectified linear units is equivalent to $\sum_{ij} \frac{\partial h_i}{\partial x_j} \cdot x_j$ where $i$ runs over the hidden units and $j$ runs over the inputs.  
\begin{align*}
...
\begin{figure}[ptb] %p
  \begin{center}
    \begin{tabular}{p{0.04in}p{2.4in}p{0.04in}p{2.4in}}
      ...
      \parbox[b]{0in}{(c) \vspace{3.8cm}} & \includegraphics[width=2.5in, keepaspectratio=true]{scat_prob_of_second_iter_activation.png} &
      \parbox[b]{0in}{(d) \vspace{3.8cm}} & \includegraphics[width=2.5in, keepaspectratio=true]{scat_weighted_average_categoricalness.png} \\
...
The classification approach used by recurrent sparse autoencoders is different from one based upon traditional sparse coding decomposition: it projects into the space of deviations from a template, which is not the same as the space of template-free parts, as is clear from figure~\ref{gradual_reconstruction_figure}(a,b).  For instance, a $5$ can easily be constructed using the parts of a $6$, making it difficult to distinguish the two.  Indeed, the first seven progressive reconstruction steps of the $6$ in figure~\ref{gradual_reconstruction_figure}(a) could just as easily be used to produce a $5$.  However, starting from a $6$ template, the parts required to break the bottom loop are outside the data manifold of the $6$ class, and so will tend to change the active template.  
